...
implementation
uses MathMy,Unit1,Unit2,graph_0d;
...
begin
  ...
  if ifC[11]=1 then gam[1]     :=g[iC_ig[11]];
  if ifC[12]=1 then gGABB[1]   :=g[iC_ig[12]];
  if ifC[13]=1 then gGAB3[2]   :=g[iC_ig[13]];
  if ifC[14]=1 then tSt        :=g[iC_ig[14]];
  if ifC[15]=1 then VNMDA      :=g[iC_ig[15]];
  if ifC[16]=1 then alAMPA[i3] :=g[iC_ig[16]];
  if ifC[17]=1 then alGABA[i3] :=g[iC_ig[17]];
  if ifC[18]=1 then alNMDA[i3] :=g[iC_ig[18]];
  if ifC[19]=1 then beAMPA[i3] :=g[iC_ig[19]];
  if ifC[20]=1 then beGABA[i3] :=g[iC_ig[20]];
  if ifC[21]=1 then beNMDA[i3] :=g[iC_ig[21]];
  if ifC[22]=1 then Iext       :=g[iC_ig[22]];
  if ifC[23]=1 then sgm_V[i3]  :=g[iC_ig[23]];
  if ifC[24]=1 then Square[i3] :=g[iC_ig[24]];
  if ifC[25]=1 then gNa[1]     :=g[iC_ig[25]];
  if ifC[26]=1 then gK[1]      :=g[iC_ig[26]];
  if ifC[27]=1 then roI        :=g[iC_ig[27]];
  if ifC[28]=1 then tau_m[i3]  :=g[iC_ig[28]];
  if ifC[29]=1 then VAMPA      :=g[iC_ig[29]];
  if ifC[30]=1 then VL[1]      :=g[iC_ig[30]];
  if ifC[31]=1 then shift_Smp  :=g[iC_ig[31]];
  if ifC[32]=1 then roE        :=g[iC_ig[32]];
  { My definitions }
{  MyDefinitions;}
end;
...
begin
  ...
  if ifC[11]=1 then g[iC_ig[11]]:=gam[1]    ;
  if ifC[12]=1 then g[iC_ig[12]]:=gGABB[1]  ;
  if ifC[13]=1 then g[iC_ig[13]]:=gGAB3[2]  ;
  if ifC[14]=1 then g[iC_ig[14]]:=tSt       ;
  if ifC[15]=1 then g[iC_ig[15]]:=VNMDA     ;
  if ifC[16]=1 then g[iC_ig[16]]:=alAMPA[i3];
  if ifC[17]=1 then g[iC_ig[17]]:=alGABA[i3];
  if ifC[18]=1 then g[iC_ig[18]]:=alNMDA[i3];
  if ifC[19]=1 then g[iC_ig[19]]:=beAMPA[i3];
  if ifC[20]=1 then g[iC_ig[20]]:=beGABA[i3];
  if ifC[21]=1 then g[iC_ig[21]]:=beNMDA[i3];
  if ifC[22]=1 then g[iC_ig[22]]:=Iext      ;
  if ifC[23]=1 then g[iC_ig[23]]:=sgm_V[i3] ;
  if ifC[24]=1 then g[iC_ig[24]]:=Square[i3];
  if ifC[25]=1 then g[iC_ig[25]]:=gNa[1]    ;
  if ifC[26]=1 then g[iC_ig[26]]:=gK[1]     ;
  if ifC[27]=1 then g[iC_ig[27]]:=roI       ;
  if ifC[28]=1 then g[iC_ig[28]]:=tau_m[i3] ;
  if ifC[29]=1 then g[iC_ig[29]]:=VAMPA     ;
  if ifC[30]=1 then g[iC_ig[30]]:=VL[1]     ;
  if ifC[31]=1 then g[iC_ig[31]]:=shift_Smp ;
  if ifC[32]=1 then g[iC_ig[32]]:=roE       ;
  { Check the limits }
  for i:=1 to mC do begin
      if ifC[i]=1 then
